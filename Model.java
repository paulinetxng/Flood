...
class Model {
    ...
    void forNeighbors(Place place, Consumer<Place> action) {
        ...
        int c = place.col;
        if (r != 0) {
            action.accept(place.pl(r - 1, c));
        }
        if (r != _height - 1) {
            action.accept(place.pl(r + 1, c));
        }
        if (c != 0) {
            action.accept(place.pl(r, c - 1));
        }
        if (c != _width - 1) {
            action.accept(place.pl(r, c + 1));
        }
    ...
    boolean[] colorsPresent() {
        ...
        for (int co = 0; co < ncolors(); co += 1) {
            colors[co] = false;
            for (int r = 0; r < _height; r += 1) {
                for (int c = 0; c < _width; c += 1) {
                    if (_cells[r][c] == co) {
                        colors[co] = true;
                    }
                }
    ...
    Set<Place> adjacentCells(int color) {
        HashSet<Place> orig = new HashSet<>(_active);
